...
public class SystemInitializer {
	...
	public static void initDataSource(){
		...
		for (HierarchicalConfiguration jdbc : cc) {
    	    ...
    	    int maxActive =setIntDBCP(jdbc,"maxActive" , GenericObjectPoolConfig.DEFAULT_MAX_TOTAL);
    	    int maxIdle = setIntDBCP(jdbc, "maxIdle", GenericObjectPoolConfig.DEFAULT_MAX_IDLE);
    	    int minIdle = setIntDBCP(jdbc, "minIdle",GenericObjectPoolConfig.DEFAULT_MIN_IDLE);
    	    int initialSize = setIntDBCP(jdbc, "initialSize",0);
    	    long maxWait = setLongDBCP(jdbc,"maxWait", GenericObjectPoolConfig.DEFAULT_MAX_WAIT_MILLIS);
    	    boolean poolPreparedStatements =setBoolDBCP(jdbc,"poolPreparedStatements", false);
    	    int maxOpenPreparedStatements = setIntDBCP(jdbc,"maxOpenPreparedStatements",GenericKeyedObjectPoolConfig.DEFAULT_MAX_TOTAL);
    	    boolean testOnBorrow =setBoolDBCP(jdbc,"testOnBorrow", true);
    	    boolean testOnReturn = setBoolDBCP(jdbc,"testOnReturn",false);
    	    long timeBetweenEvictionRunsMillis =  setLongDBCP(jdbc,"timeBetweenEvictionRunsMillis",GenericObjectPoolConfig.DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS);
    	    int numTestsPerEvictionRun = setIntDBCP(jdbc,"numTestsPerEvictionRun",GenericObjectPoolConfig.DEFAULT_NUM_TESTS_PER_EVICTION_RUN); 
    	    long minEvictableIdleTimeMillis =   setLongDBCP(jdbc,"minEvictableIdleTimeMillis",GenericObjectPoolConfig.DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS);
